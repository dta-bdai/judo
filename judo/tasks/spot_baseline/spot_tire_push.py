"""Spot tire manipulation task built on the locomotion baseline."""
...
@dataclass
class SpotTirePushBaselineConfig(SpotLocomotionConfig):
    """Config for the Spot tire manipulation task built on locomotion."""
    ...
    tire_goal_position: np.ndarray = np_1d_field(
...
class SpotTirePushBaseline(SpotLocomotion):
    """Spot locomotion task augmented with tire manipulation rewards."""

    name: str = "spot_tire_push_baseline"
    config_t: type[SpotTirePushBaselineConfig] = SpotTirePushBaselineConfig
    ...
    def reward(
        ...
        sensors: np.ndarray,
        controls: np.ndarray,
        config: SpotTirePushBaselineConfig,
        system_metadata: dict[str, Any] | None = None,
    ) -> np.ndarray:
        """Compose locomotion rewards with tire manipulation objectives."""
        ...
        tire_goal = np.asarray(config.tire_goal_position, dtype=np.float64)
        object_goal_error = np.linalg.norm(object_pos - tire_goal, axis=-1)
    ...
    def success(self, model: MjModel, data: MjData, config: SpotTirePushBaselineConfig, metadata: dict[str, Any] | None = None) -> bool:
        ...
        goal_pos = np.array(config.tire_goal_position)
    ...
    def failure(self, model: MjModel, data: MjData, config: SpotTirePushBaselineConfig, metadata: dict[str, Any] | None = None) -> bool:
