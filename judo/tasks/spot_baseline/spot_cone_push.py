"""Spot cone manipulation task built on the locomotion baseline."""
...
@dataclass
class SpotConePushBaselineConfig(SpotLocomotionConfig):
    """Config for the Spot cone manipulation task built on locomotion."""
    ...
    cone_goal_position: np.ndarray = np_1d_field(
...
class SpotConePushBaseline(SpotLocomotion):
    """Spot locomotion task augmented with cone manipulation rewards."""

    name: str = "spot_cone_push_baseline"
    config_t: type[SpotConePushBaselineConfig] = SpotConePushBaselineConfig
    ...
    def reward(
        ...
        sensors: np.ndarray,
        controls: np.ndarray,
        config: SpotConePushBaselineConfig,
        system_metadata: dict[str, Any] | None = None,
    ) -> np.ndarray:
        """Compose locomotion rewards with cone manipulation objectives."""
        ...
        cone_goal = np.asarray(config.cone_goal_position, dtype=np.float64)
        object_goal_error = np.linalg.norm(object_pos - cone_goal, axis=-1)
    ...
    def success(self, model: MjModel, data: MjData, config: SpotConePushBaselineConfig, metadata: dict[str, Any] | None = None) -> bool:
        ...
        goal_pos = np.array(config.cone_goal_position)
    ...
    def failure(self, model: MjModel, data: MjData, config: SpotConePushBaselineConfig, metadata: dict[str, Any] | None = None) -> bool:
