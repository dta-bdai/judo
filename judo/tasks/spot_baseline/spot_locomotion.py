...
class SpotLocomotion(SpotBase[SpotLocomotionConfig]):
    ...
    def __init__(
        ...
        config: SpotLocomotionConfig | None = None,
    ) -> None:
    ...
    def reward(
        ...
        upright_cost = -self.config.w_upright * np.mean(
            np.sum(upright_error**2, axis=-1),
            axis=1,
        )

        # Height cost (matching A1)
        height_error = (torso_pos[..., 2] - avg_foot_height) - self.config.height_goal
        height_cost = -self.config.w_height * np.mean(height_error**2, axis=1)

        # Position cost (matching A1)
        goal = np.asarray(self.config.goal_position, dtype=np.float64)
        pos_error = head_pos - goal
        position_cost = -self.config.w_position * np.mean(
        ...
        schedule = self._foot_schedule_a1(phase, self.config)[None, :, :]
        ...
        posture_cost = -self.config.w_posture * np.mean(
        ...
        arm_posture_cost = -self.config.w_arm_posture * np.mean(
            np.sum(arm_error**2, axis=-1),
            axis=1,
        )

        # Effort cost (matching A1)
        effort_cost = -self.config.w_effort * np.mean(
