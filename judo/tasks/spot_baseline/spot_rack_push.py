"""Spot rack manipulation task built on the locomotion baseline."""
...
@dataclass
class SpotRackPushBaselineConfig(SpotLocomotionConfig):
    """Config for the Spot rack manipulation task built on locomotion."""
    ...
    rack_goal_position: np.ndarray = np_1d_field(
...
class SpotRackPushBaseline(SpotLocomotion):
    """Spot locomotion task augmented with rack manipulation rewards."""

    name: str = "spot_rack_push_baseline"
    config_t: type[SpotRackPushBaselineConfig] = SpotRackPushBaselineConfig
    ...
    def reward(
        ...
        sensors: np.ndarray,
        controls: np.ndarray,
        config: SpotRackPushBaselineConfig,
        system_metadata: dict[str, Any] | None = None,
    ) -> np.ndarray:
        """Compose locomotion rewards with rack manipulation objectives."""
        ...
        fallen = (body_height <= config.spot_fallen_threshold).any(axis=1)
        fall_penalty = -config.fall_penalty * fallen.astype(np.float64)

        rack_goal = np.asarray(config.rack_goal_position, dtype=np.float64)
        object_goal_error = np.linalg.norm(object_pos - rack_goal, axis=-1)
        goal_reward = -config.w_goal * np.mean(object_goal_error, axis=1)

        alignment = np.sum(object_y_axis * Z_AXIS, axis=-1)
        orientation_violation = (np.abs(alignment) < config.orientation_threshold).astype(np.float64)
    ...
    def success(self, model: MjModel, data: MjData, config: SpotRackPushBaselineConfig, metadata: dict[str, Any] | None = None) -> bool:
        """Check if the rack is in the goal position."""
        object_pos = data.qpos[..., self.object_pose_idx[0:3]]
        object_vel = data.qvel[..., self.object_vel_idx[0:3]]
        goal_pos = np.array(config.rack_goal_position)
    ...
    def failure(self, model: MjModel, data: MjData, config: SpotRackPushBaselineConfig, metadata: dict[str, Any] | None = None) -> bool:
