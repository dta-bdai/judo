...
XML_PATH = str(MODEL_PATH / "xml/spot_door_box.xml")


@dataclass
class SpotDoorBoxConfig(TaskConfig):
    """Configuration for Spot Door Box task."""
...
class SpotDoorBox(Task[SpotDoorBoxConfig]):
    """Spot Door Box task."""

    name: str = "spot_door_box"
    config_t: type[SpotDoorBoxConfig] = SpotDoorBoxConfig

    def __init__(
        self,
        model_path: str = XML_PATH,
        sim_model_path: str | None = None,
        config: SpotDoorBoxConfig | None = None,
    ) -> None:
        """Initializes the spot door box task."""
    ...
    def reward(
        ...
        """
        Compute reward for spot door box task.
        ...
        upright_rew = -self.config.w_upright * smooth_l1_norm(base_quat[..., 0] - 1, self.config.p_upright).sum(-1)

        # Height reward: penalize deviation from desired height (around 0.7m)
        desired_height = 0.7
        height_rew = -self.config.w_height * smooth_l1_norm(base_pos[..., 2] - desired_height, self.config.p_height).sum(-1)
        ...
        lateral_rew = -self.config.w_lateral * smooth_l1_norm(lateral_distance, self.config.p_lateral).sum(-1)

        # Velocity penalty: penalize excessive motion
        velocity_rew = -self.config.w_velocity * (
    ...
    def reset(self) -> None:
        """Resets the spot door box task."""
        # Reset the robot position to be away from the door
        self.data.qpos[0] = -1.0  # Move robot back in x direction
        self.data.qpos[1] = 0.0  # Center in y direction
        self.data.qpos[2] = 1.5  # Set z position to 1.5 meters

        # Find and reset the door joint to 0 angle
        # mjOBJ_JOINT = 3 in mujoco
        door_joint_id = mujoco.mj_name2id(self.model, 3, "door_hinge")
        if door_joint_id >= 0:
            door_qpos_addr = self.model.jnt_qposadr[door_joint_id]
            self.data.qpos[door_qpos_addr] = 0.0  # Set door to closed position (0 angle)
