...
class SpotTireStack(SpotBase[SpotTireStackConfig]):
    ...
    def reward(
        ...
        tire_stack_xy_reward = -config.w_tire_stack_xy * xy_distance.mean(axis=-1)
        ...
        tire_stack_z_reward = -config.w_tire_stack_z * z_error.mean(axis=-1)
        ...
        orientation_active = tire_distance < config.orientation_reward_distance_threshold
        ...
        tire_orientation_reward = np.where(
            orientation_active_any, -config.w_tire_orientation * orientation_error.mean(axis=-1), 0.0
        )
        ...
        bottom_tire_velocity_reward = -config.w_bottom_tire_velocity * (
        ...
        gripper_height_penalty = -config.w_gripper_height_penalty * gripper_height_excess.mean(axis=-1)
    ...
    def success(self, model: MjModel, data: MjData, config: SpotTireStackConfig, metadata: dict[str, Any] | None = None) -> bool:
