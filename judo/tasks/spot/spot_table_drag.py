...
class SpotTableDrag(SpotBase[SpotTableDragConfig]):
    ...
    config_t: type[SpotTableDragConfig] = SpotTableDragConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotTableDragConfig | None = None) -> None:
    ...
    def reward(
        ...
        reward += -self.config.w_goal * np.linalg.norm(
            W_p_table[..., :2] - np.array(self.config.goal_position[:2])[None, None], axis=-1
        ).mean(-1)
        ...
        reward += -self.config.w_gripper_orientation * (1.0 - np.abs(dot_product)).mean(axis=-1)

        # Table orientation reward - keep table upright
        reward += -self.config.w_table_orientation * (1.0 - np.maximum(table_z_axis[..., 2].mean(axis=-1), 0.0))
    ...
    def success(self, model: MjModel, data: MjData, metadata: dict[str, Any] | None = None) -> bool:
        ...
        position_error = np.linalg.norm(object_pos[:2] - self.config.goal_position[:2])
        position_success = position_error <= self.config.position_tolerance
