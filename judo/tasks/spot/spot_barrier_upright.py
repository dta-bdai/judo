...
class SpotBarrierUpright(SpotBase[SpotBarrierUprightConfig]):
    ...
    name: str = "spot_barrier_upright"
    config_t: type[SpotBarrierUprightConfig] = SpotBarrierUprightConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotBarrierUprightConfig | None = None) -> None:
        ...
        self.gripper_to_grasp_left_idx = get_sensor_indices(self.model, "sensor_gripper_to_grasp_left")
        self.gripper_to_grasp_right_idx = get_sensor_indices(self.model, "sensor_gripper_to_grasp_right")
    ...
    def reward(
        ...
        gripper_x_axis_norm = gripper_x_axis / (
            np.linalg.norm(gripper_x_axis, axis=-1, keepdims=True) + 1e-8
        )
        object_x_axis_norm = object_x_axis / (
            np.linalg.norm(object_x_axis, axis=-1, keepdims=True) + 1e-8
        )
        x_dot_product = np.abs(np.sum(gripper_x_axis_norm * object_x_axis_norm, axis=-1))
        x_alignment_error = 1.0 - x_dot_product

        gripper_y_axis_norm = gripper_y_axis / (
            np.linalg.norm(gripper_y_axis, axis=-1, keepdims=True) + 1e-8
        )
        object_z_axis_norm = object_z_axis / (
            np.linalg.norm(object_z_axis, axis=-1, keepdims=True) + 1e-8
        )
        ...
        gripper_orientation_reward = (
            -self.config.w_gripper_orientation * total_orientation_error.mean(axis=-1)
        )
        ...
        gripper_joint_pos = qpos[..., 1:, self.gripper_joint_idx]  # Actual position (0.0=closed, -1.54=open), skip initial state
