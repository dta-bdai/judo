...
class SpotBoxPush(SpotBase):
    ...
    config_t: type[SpotBoxPushConfig] = SpotBoxPushConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotBoxPushConfig | None = None) -> None:
        super().__init__(model_path=model_path, use_legs=USE_LEGS, config=config)
    ...
    def reward(
        ...
        object_orientation_reward = -self.config.w_orientation * np.abs(
    ...
    def failure(self, model: MjModel, data: MjData, config: SpotBoxPushConfig, metadata: dict[str, Any] | None = None) -> bool:
