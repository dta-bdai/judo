...
@dataclass
class SpotTireRackDragConfig(SpotBaseConfig):
    ...
    goal_position: np.ndarray = np_1d_field(
        ...
        maxs=[5.0, 5.0, 2.0],
        steps=[0.1, 0.1, 0.1],
        vis_name="tire_rack_goal_position",
        xyz_vis_indices=[0, 1, 2],
        xyz_vis_defaults=[-1.0, 1.3, 0.0],
    )
    w_fence: float = 1000.0
    w_goal: float = 500.0
    w_orientation: float = 50.0  # Reward for keeping rack aligned with global x, y, z axes
    w_heading_alignment: float = 500.0  # Reward for rack facing towards goal
    object_fallen_threshold: float = 0.15  # Tire rack center of mass height
...
class SpotTireRackDrag(SpotBase[SpotTireRackDragConfig]):
    """Task getting Spot to drag a tire rack to the origin (goal location)."""
    ...
    def __init__(self, model_path: str = XML_PATH) -> None:
        ...
        # Grasp point proximity sensors (left_mid only)
        self.gripper_to_grasp_left_mid_idx = get_sensor_indices(self.model, "sensor_gripper_to_grasp_left_mid")
        ...
        self.torso_to_approach_mid_idx = get_sensor_indices(self.model, "sensor_torso_to_approach_mid")
    ...
    def reward(
        ...
        sensors: np.ndarray,
        controls: np.ndarray,
        config: SpotTireRackDragConfig,
        system_metadata: dict[str, Any] | None = None,
    ) -> np.ndarray:
        """Reward function for the Spot tire rack dragging task."""
        ...
        # Grasp point proximity vector
        gripper_to_grasp_left_mid = sensors[..., self.gripper_to_grasp_left_mid_idx]

        # Gripper orientation axes
        gripper_x_axis = sensors[..., self.gripper_x_axis_idx]
        gripper_y_axis = sensors[..., self.gripper_y_axis_idx]
        gripper_z_axis = sensors[..., self.gripper_z_axis_idx]
        ...
        torso_to_approach_mid = sensors[..., self.torso_to_approach_mid_idx]
        ...
        # Orientation reward - keep rack aligned with global x, y, z axes
        ...
        # Heading alignment reward - keep rack's x-axis pointing towards goal
        # Note: object_pos from qpos includes initial state, but object_x_axis from sensors doesn't
        rack_to_goal = np.array(config.goal_position)[None, None, :2] - object_pos[..., 1:, :2]  # Skip initial state
        rack_to_goal_norm = rack_to_goal / (np.linalg.norm(rack_to_goal, axis=-1, keepdims=True) + 1e-8)
        ...
        heading_alignment = np.sum(object_x_horizontal_norm * rack_to_goal_norm, axis=-1)
        ...
        approach_distances = np.stack([distance_to_left_approach, distance_to_mid_approach, distance_to_right_approach], axis=-1)
        ...
        distance_to_left_mid = np.linalg.norm(gripper_to_grasp_left_mid, axis=-1)
        gripper_to_grasp_proximity_reward = -config.w_gripper_to_grasp_proximity * distance_to_left_mid.mean(-1)

        # Gripper orientation reward - only require gripper z-axis to align with object z-axis (both upright)
        ...
        gripper_orientation_reward = -config.w_gripper_orientation * z_alignment_error.mean(axis=-1)
    ...
    def reset_pose(self) -> np.ndarray:
        """Reset pose of robot and object - tire rack starts upright."""
        # Tire rack starts upright (no rotation) at elevated position (z=0.3)
        reset_object_pose = np.array([*DEFAULT_OBJECT_POS, 0.3, 1, 0, 0, 0])
    ...
    def success(self, model: MjModel, data: MjData, config: SpotTireRackDragConfig, metadata: dict[str, Any] | None = None) -> bool:
        """Check if the tire rack has been successfully dragged to the goal position."""
