...
class SpotTireRackDrag(SpotBase[SpotTireRackDragConfig]):
    ...
    name: str = "spot_tire_rack_drag"
    config_t: type[SpotTireRackDragConfig] = SpotTireRackDragConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotTireRackDragConfig | None = None) -> None:
    ...
    def reward(
        ...
        rack_to_goal = np.array(self.config.goal_position)[None, None, :2] - object_pos[..., 1:, :2]  # Skip initial state
        ...
        gripper_to_grasp_proximity_reward = -self.config.w_gripper_to_grasp_proximity * distance_to_left_mid.mean(-1)
