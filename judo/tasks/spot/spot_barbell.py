...
class SpotBarbell(SpotBase[SpotBarbellConfig]):
    ...
    name: str = "spot_barbell"
    config_t: type[SpotBarbellConfig] = SpotBarbellConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotBarbellConfig | None = None) -> None:
        super().__init__(model_path=model_path, use_legs=USE_LEGS, use_arm=True, use_gripper=True, config=config)
    ...
    def reward(
        ...
        goal_reward = -self.config.w_goal * np.linalg.norm(
            W_p_barbell - np.array(self.config.goal_position)[None, None], axis=-1
        ).mean(-1)
        ...
        goal_height_reward = -self.config.w_goal_height * goal_height_error
        ...
        torso_proximity_reward = -self.config.w_torso_proximity * torso_t_barbell_norm.mean(axis=-1)
        ...
        barbell_orientation_reward = -self.config.w_barbell_orientation * orientation_error.mean(axis=-1)
        ...
        gripper_barbell_orientation_reward = -self.config.w_gripper_barbell_orientation * (1.0 - np.abs(dot_product)).mean(axis=-1)
        ...
        is_grasping = (gripper_distance < self.config.grasp_distance_threshold) & (barbell_height > self.config.grasp_height_threshold)
        ...
        gripper_close_reward = -self.config.w_gripper_close * ((1.0 - gripper_closeness) * is_grasping).mean(axis=-1)
