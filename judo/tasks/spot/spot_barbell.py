...
RESET_OBJECT_POSE = np.array([3, 0, 0.15, 1, 0, 0, 0])
...
class SpotBarbellConfig(SpotBaseConfig):
    """Config for the spot barbell manipulation task."""
...
class SpotBarbell(SpotBase):
    """Task getting Spot to move a barbell to a desired goal location."""
    ...
    def reward(
        ...
        """Reward function for the Spot barbell moving task."""
        ...
        barbell_orientation_reward = -config.w_orientation * np.abs(
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        reset_object_pose = np.array([*object_pos, 0.15, 1, 0, 0, 0])
