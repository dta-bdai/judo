...
class SpotTrafficCone(SpotBase[SpotTrafficConeConfig]):
    ...
    name: str = "spot_traffic_cone"
    config_t: type[SpotTrafficConeConfig] = SpotTrafficConeConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotTrafficConeConfig | None = None) -> None:
    ...
    def success(
        ...
        velocity_success = np.linalg.norm(data.qvel[..., self.object_vel_idx[0:3]], axis=-1) < self.config.velocity_tolerance
