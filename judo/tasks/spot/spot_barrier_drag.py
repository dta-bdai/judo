...
class SpotBarrierDrag(SpotBase[SpotBarrierDragConfig]):
    ...
    name: str = "spot_barrier_drag"
    config_t: type[SpotBarrierDragConfig] = SpotBarrierDragConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotBarrierDragConfig | None = None) -> None:
    ...
    def reward(
        ...
        barrier_to_goal = np.array(self.config.goal_position)[None, None, :2] - object_pos[..., 1:, :2]  # Skip initial state to match sensors
