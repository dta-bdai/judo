...
RESET_OBJECT_POSE = np.array([3, 0, 0.52, 1, 0, 0, 0])
...
@dataclass
class SpotTableShortConfig(SpotBaseConfig):
    """Config for the spot table_short manipulation task."""
...
class SpotTableShort(SpotBase[SpotTableShortConfig]):
    """Task getting Spot to move a short table to a desired goal location."""
    ...
    def __init__(self, model_path: str = XML_PATH) -> None:
        ...
        self.object_vel_idx = get_vel_indices(self.model, ["table_short_joint"])
    ...
    def reward(
        ...
        sensors: np.ndarray,
        controls: np.ndarray,
        config: SpotTableShortConfig,
        system_metadata: dict[str, Any] | None = None,
    ) -> np.ndarray:
        """Reward function for the Spot table_short moving task."""
        ...
        # Compute l2 distance from gripper to object pos.
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        reset_object_pose = np.array([*object_pos, 0.52, 1, 0, 0, 0])
    ...
    def success(self, model: MjModel, data: MjData, config: SpotTableShortConfig, metadata: dict[str, Any] | None = None) -> bool:
        ...
        # Check if object is within position tolerance of goal
        position_error = np.linalg.norm(object_pos - config.goal_position)
