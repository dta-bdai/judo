...
class SpotRuggedBoxPush(SpotBase[SpotRuggedBoxPushConfig]):
    """Task getting Spot to push a rugged box to a desired goal location."""

    name: str = "spot_rugged_box_push"
    config_t: type[SpotRuggedBoxPushConfig] = SpotRuggedBoxPushConfig

    def __init__(self, model_path: str = XML_PATH, config: SpotRuggedBoxPushConfig | None = None) -> None:
        ...
        self.object_pose_idx = get_pos_indices(self.model, ["rugged_box_joint"])
        self.object_vel_idx = get_vel_indices(self.model, ["rugged_box_joint"])
    ...
    def reward(
        ...
        object_pos_no_init = object_pos[..., 1:, :]
        ...
        box_orientation_reward = -self.config.w_orientation * (
        ...
        box_to_goal_2d = goal_position[None, None, :2] - object_pos_no_init[..., :2]
        ...
        torso_desired_xy = object_pos_no_init[..., :2] - distance_from_box * box_to_goal_unit
        torso_desired = np.concatenate([torso_desired_xy, object_pos_no_init[..., 2:3]], axis=-1)
        ...
        torso_proximity_reward = -self.config.w_torso_proximity * np.linalg.norm(
            body_pos_no_init - torso_desired, axis=-1
        ).mean(-1)
